...
from .lon_values import is_writable_time_datapoint
...
def _time_unique_ids(host: str) -> set[str]:
    """Return unique_ids for datapoints that moved to the time platform."""
    ids: set[str] = set()
    for dp in _load_all_datapoints():
        oid = str(dp.get("oid", ""))
        key = str(dp.get("key", ""))
        if not oid or not key:
            continue
        if is_writable_time_datapoint(dp):
            ids.add(_lon_unique_id(host, oid, key))
    return ids
...
async def async_migrate_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    if config_entry.version < 3:
        _LOGGER.info(
            "Migrating Windhager config entry %s to version 3 (time platform cleanup)",
            config_entry.entry_id,
        )
        control_ids |= _time_unique_ids(host)
